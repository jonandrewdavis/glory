...
func check_arrows_and_ram() -> void:
	...
	expect(world.scoreboard.entries[1].kills == 0 and world.scoreboard.entries[1].deaths == 0, "Creep death awards no player kill or death")
	enemy.health.take_damage(200, player)
	expect(world.scoreboard.entries[1].kills == 0, "Damage after death cannot award another kill")
	...
	expect(world.scoreboard.entries[1].kills == 0, "A creep's killing blow does not credit a player who damaged the victim earlier")
	await reset_creeps()
