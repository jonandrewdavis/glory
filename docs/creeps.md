...
marker, immediately outside its gate. Gate damage remains deferred.

## Behavior and tuning

- `entities/creep.tscn`: 200 health, no regeneration and no respawn.
- `entities/creep.gd`: 60 px/s, 20-pixel horizontal melee reach, independent uniform
...
reach and line of sight again. Vertical reach accounts for the soldiers' maximum
walkable slope, allowing adjacent enemies to fight on steep ramps while terrain
still blocks strikes. Soldiers use gravity and small terrain hops;
horizontal space reservations prevent passing or climbing over another soldier.
...
damage, player pass-through, ram counts, and cleanup.
