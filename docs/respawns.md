...
| Band | Blue moving right captures at | Orange moving left captures at |
| --- | --- | --- |
| 1 Blue fortress | Permanent Blue | Never |
| 2 Blue outpost | x=-608 (reclaim) | Blue gate endpoint |
| 3 Left center | x=608 | x=-800 |
| 4 Right center | x=800 | x=-608 |
| 5 Orange outpost | Orange gate endpoint | x=608 (reclaim) |
| 6 Orange fortress | Never | Permanent Orange |

Capture is immediate on crossing a threshold in the attacking direction.
Ownership persists until recaptured and never returns to neutral. The most
forward owned band is active: highest order for Blue, lowest for Orange.
...
milestones monotonically in each team's attack direction. The editor reports
...
original team markers, with the new timing and protection.

## Protection and lifecycle

Every fresh spawn gets two seconds of server-controlled damage protection,
shown by a pale ring. Protection ends on an accepted shot or shield reflection.
Protected players do not push the ram. Creeps retain their existing influence
and spawn behavior. Loading a map does not consume a remote player's protection.
...
respawn positions, protection, persistent scores, a reload while dead, live
